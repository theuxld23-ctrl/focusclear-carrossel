...
- 📖 **Briefing Mestre v1.1** (fonte de verdade do produto) em `docs/BRIEFING.md`. Roadmap: FASE 3 = motor de vídeo/Reel (roteirista_video + ElevenLabs + HeyGen + Palmier), FASE 4 = Tendências+Motion, FASE 5 = Pilares/Personagem/Config (UI ✅ já feita aqui), FASE 6 = produto-ready.
- ⚠️ Divergências briefing×código a reconciliar: (1) pilar `novela_reality` (em `pilares.json`) vs "Cultura Pop / Fofoca" separado de Novela (briefing decisão travada); (2) brasa `#E8472B` (compositor) vs `#FF5436` (briefing). Não alterados — `engine/data` é fonte de verdade e o briefing pede registrar a decisão antes de virar código.
- Próxima: ativar as APIs no `.env` para a 1ª execução real ponta-a-ponta; depois FASE 3 (Reel).
